 ...
 */
public class MsPacManIntroScene2 implements PacManGameScene {
	...
	public MsPacManIntroScene2(PacManGame game, V2i size, MsPacManAssets assets) {
		this.game = game;
		this.size = size;
		this.assets = assets;
	}
	...
	private void drawGhostsChasingPacMan(Graphics2D g) {
		...
		g.drawImage(pacWalkingSprite(LEFT), (int) pacManX, y, null);
	...
	private void drawPacManChasingGhosts(Graphics2D g) {
		...
		g.drawImage(pacWalkingSprite(RIGHT), (int) pacManX, y, null);
		if (pacManX < size.x) {
			pacManX += 0.6f;
			leftmostGhostX += 0.3f;
		}
	...
	private BufferedImage pacWalkingSprite(Direction dir) {
